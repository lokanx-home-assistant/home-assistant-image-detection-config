...
DOMAIN = "python_script_x"

FOLDER = "python_scripts_x"
...
def discover_scripts(hass):
    ...
    def python_script_service_handler(call):
        ...
        execute_script_x(hass, call.service, call.data)
    ...
    # Load user-provided service descriptions from python_scripts_x/services.yaml
...
@bind_hass
def execute_script_x(hass, name, data=None):
    """Execute a script."""
    filename = f"{name}.py"
    with open(hass.config.path(FOLDER, sanitize_filename(filename))) as fil:
        source = fil.read()
    execute_x(hass, filename, source, data)


@bind_hass
def execute_x(hass, filename, source, data=None):
